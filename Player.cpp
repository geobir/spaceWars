...
Player::Player(void) : Entity( _x, _y, _sizex, _sizey, _hp, _max_hp, _dam, _graph, _dir, _speed, _score), _score(50), _invincible(0)
{
	return ;
}

Player::Player(int x, int y, int sizex, int sizey, int hp, int max_hp, int dam, std::string graph, int dir, float speed, int score) :
			Entity(x, y, sizex, sizey, hp, max_hp, dam, graph, dir, speed, score), _invincible(0)
{
	return ;
}

Player::Player(Player const &src) : Entity(src), _invincible(0)
{
	*this = src;
	return ;
}
